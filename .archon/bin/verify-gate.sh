...
case "$PHASE" in
  ...
  tdd)
    ...
    ;;
  implement|refactor)
    if command -v npm >/dev/null 2>&1 && [[ -f package.json ]]; then npm test; fi
    if command -v bun >/dev/null 2>&1 && [[ -f package.json ]]; then bun test; fi
    echo "${PHASE} gate OK"
    ;;
  pr)
    if command -v gh >/dev/null 2>&1; then
      gh pr view --json url -q .url >/dev/null 2>&1 || { echo "No open PR found" >&2; exit 1; }
    fi
    echo "PR gate OK"
    ;;
  smoke)
